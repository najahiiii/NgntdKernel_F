				      ...
				      char *buf, int size)
{
	...
	if (ret > 0 && gen->istatus == CBS_EXCEPTION) {
		...
		snprintf(buf, size,
			...
			"excdet0 0x%lx, excdet1 0x%x", raw_smp_processor_id(),
...
static int quicktest0(unsigned long arg)
{
	...
	if (word0 != word1 || word1 != MAGIC) {
		printk(KERN_DEBUG
		     ...
		     raw_smp_processor_id(), word1, MAGIC);
		goto done;
	}
	ret = 0;
...
static int quicktest1(unsigned long arg)
{
	...
	if (i != 4) {
		printk(KERN_DEBUG "GRU:%d quicktest2: bad message, i %d, m %p, m8 %d\n",
			raw_smp_processor_id(), i, m, m ? m[8] : -1);
		goto done;
	}
	ret = 0;
	...
	do {
		...
		if (istatus != CBS_IDLE) {
			...
			printk(KERN_DEBUG "GRU:%d quicktest2:cb %d,  buf 0x%lx, 0x%lx, 0x%lx, 0x%lx\n",
			       raw_smp_processor_id(), i, buf[4 * i], buf[4 * i + 1], buf[4 * i + 2], buf[4 * i + 3]);
			ret = -EIO;
		}
		k--;
		gen = cb;
